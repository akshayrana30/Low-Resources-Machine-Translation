...
def train_model(emb_dim, data):
    ...
    model.train(sentences=[x.split(" ") for x in data],
...
def get_embedding_matrix(tokenizer, word_vector, vocab_size, emb_size=256):
    ...
    for (batch, (index, word)) in enumerate(tokenizer.index_word.items()):
        if batch + 1 == vocab_size:
            break
        embedding_matrix[index] = word_vector[word] if word in word_vector else np.zeros(
            emb_size, dtype="float32")
...
if __name__ == "__main__":
    ...
    emb = get_embedding_matrix(
        input_tokenizer, word_vectors, vocab_size, emb_dim)
    save_emb(emb, path)
