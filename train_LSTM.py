...
from models.LSTM import LSTMEncoder, LSTMDecoder
...
def main(argv):
    ...
    encoder = LSTMEncoder(vocab_size=src_vocsize,
    ...
    @tf.function
    def train_step(inp, targ, enc_hidden, enc_c):
        train_loss = 0
        with tf.GradientTape() as tape:
            # feed input into encoder
            enc_output, enc_hidden, enc_c = encoder(inp, [enc_hidden, enc_c])
            ...
            for t in range(1, targ.shape[1]):
                pred, dec_hidden, dec_c = decoder(dec_input, [dec_hidden, dec_c])
    ...
    @tf.function
    def valid_step(inp, targ, enc_hidden, enc_c):
        val_loss = 0
        # feed input into encoder
        enc_output, enc_hidden, enc_c = encoder(inp, [enc_hidden, enc_c])
        # initial states of decoder is the final state of encoder
    ...
    for epoch in range(EPOCHS):
        start = time.time()
        enc_hidden, enc_c = encoder.initialize_hidden_state()
        total_train_loss = 0.
        total_val_loss = 0.

        # train
        for (inp, targ) in train_dataset:
            train_loss = train_step(inp, targ, enc_hidden, enc_c)
        ...
        for (inp, targ) in valid_dataset:
            val_loss = valid_step(inp, targ, enc_hidden, enc_c)
