...
flags.DEFINE_integer('batch_size', 64,
                     'batch size')
flags.DEFINE_integer('enc_units', 32,
                     'internal unit of RNN encoder')
flags.DEFINE_integer('dec_units', 32,
...
def main(argv):
    ...
    logging.info("Create RNN Model")
    encoder = RNNEncoder(vocab_size=src_vocsize,
                         embedding_dim=FLAGS.emb_dim,
                         enc_units=FLAGS.enc_units,
                         batch_sz=FLAGS.batch_size)
    decoder = RNNDecoder(vocab_size=tar_vocsize,
                         embedding_dim=FLAGS.emb_dim,
    ...
    ckpt_prefix = os.path.join(FLAGS.ckpt, "ckpt_RNN")
    ...
    for epoch in range(EPOCHS):
        ...
        for (inp, targ) in valid_dataset:
            val_loss = valid_step(inp, targ, enc_hidden)
